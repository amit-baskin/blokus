...
def get_legal_tiles(state, problem):
    ...
    if np.all(state.state == -1):
        legal_corners.add(problem.starting_point)
    else:
        for y in range(state.board_h):
            for x in range(state.board_w):
                if state.state.item((x, y)) == -1 and state.check_tile_attached(0, x, y) and state.check_tile_legal(0, x, y):
...
def is_tile_illegal(tile, state):
    tiles = [np.array(tile) for i in range(4)]
...
def blokus_corners_heuristic(state, problem):
    ...
    #     for legal_corner in legal_corners:
    #         board = np.copy(state.state)
    #         fill_tiles_in_the_way(goal, legal_corner, board)
